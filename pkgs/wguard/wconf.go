...
func PrintDevice(thisDevice *cloudflare.Device, boundDevice *cloudflare.BoundDevice) {
	tui.Green("=======================================")
	tui.Cyan(fmt.Sprintf("%-13s : %s", "Device name", *boundDevice.Name))
	tui.Cyan(fmt.Sprintf("%-13s : %s", "Device model", thisDevice.Model))
	tui.Cyan(fmt.Sprintf("%-13s : %t", "Device active", boundDevice.Active))
	tui.Cyan(fmt.Sprintf("%-13s : %s", "Account type", thisDevice.Account.AccountType))
	tui.Cyan(fmt.Sprintf("%-13s : %s", "Role", thisDevice.Account.Role))
	tui.Cyan(fmt.Sprintf("%-13s : %s", "Premium data", F32ToHumanReadable(thisDevice.Account.PremiumData)))
	tui.Cyan(fmt.Sprintf("%-13s : %s", "Quota", F32ToHumanReadable(thisDevice.Account.Quota)))
	tui.Green("=======================================")
}
