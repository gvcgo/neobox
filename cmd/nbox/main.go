...
func NewApps() (a *Apps) {
	...
	neoboxWorkDir := filepath.Join(homeDir, ".neobox")
	cnf := conf.NewNeoConf(neoboxWorkDir)
	a = &Apps{
		rootCmd: &cobra.Command{},
		conf:    cnf,
	}
...
func (that *Apps) initiate() {
	...
	script := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			binPath, _ := os.Executable()
			err := os.WriteFile(scriptPath, []byte(fmt.Sprintf("%s %s", binPath, ssCmd)), 0o777)
			if err == nil {
